    ...
    <nav className="bg-gray-900 border-b border-gray-800 sticky top-0 z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center">
          <Link
            to="/"
            className="text-2xl font-extrabold tracking-tight text-white lowercase"
          >
      ...
      {isOpen && (
        <div className="md:hidden mt-4 pt-4 border-t border-gray-800 space-y-3 flex flex-col">
          <Link
            to="/feed"
            className="text-sm font-medium text-gray-400 hover:text-white py-1"
          >
            Feed
          </Link>
          <Link
          ...
          {isAuthenticated ? (
            ...
            <div className="flex flex-col space-y-2 pt-2 border-t border-gray-800/50">
              <Link
                to="/sign-in"
                className="text-center text-sm font-medium text-gray-400 hover:text-white py-2.5 rounded-lg border border-gray-800 bg-gray-950/50"
              >
                Login
              </Link>
              <Link
                to="/sign-up"
                className="text-center bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold py-2.5 rounded-lg"
              >
                Sign Up
              </Link>
            </div>
